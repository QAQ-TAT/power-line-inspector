...
char *ptr;
long ret = 0;
int stopFlag = 1;//用于Auto自动停止不然会一直某状态运行
...
void redUDP()
{
    ...
    {
        cout << "UDP收到了" << endl;
        //bzero(AutoFlag,sizeof(AutoFlag));
        UDPret = recvfrom(UDPsockfd,AutoFlag,sizeof(AutoFlag),0,(struct sockaddr *)&cliaddr,&len);
        if(UDPret != -1)
        {
            //printf("%s:%s",(char *)inet_ntoa(cliaddr.sin_addr),AutoFlag);
            PolicyFlag[1] == AutoFlag[1];
        ...
        cout << "AutoFlag[1]=" << AutoFlag[1] << endl;
...
int main(int argc,char *argv[])
{
    ...
    fd = open("/dev/USBmagnetic", O_RDWR | O_NOCTTY );//| O_NDELAY
    ...
    while(n.ok())
    {
        ...
        else if(AutoFlag[1] == '0' && stopFlag == 1)
        {
            ret = 0;
            policy();
            stopFlag = 0;
