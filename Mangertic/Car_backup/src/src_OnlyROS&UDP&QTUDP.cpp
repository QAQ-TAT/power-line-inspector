...
void UDPsend()
{
    while(1)
    {
        ...
        else if(XQROS2UDPbuf[0] < 100 && XQROS2UDPbuf[0]!=0)
        {
            cout << "上升" << endl;
...
int main(int argc,char *argv[])
{
	...
	srvaddr.sin_family = AF_INET;
        srvaddr.sin_port = htons(/*atoi(argv[2])*/50001);//对方的端口号
        inet_pton(AF_INET,/*argv[1]*/"192.168.8.242",&srvaddr.sin_addr);//对方的IP地址
    ...
    receiver.sin_port = htons(/*atoi(argv[3])*/50000);//接收端口号(自己端口号)本程序接受用到的端口号
