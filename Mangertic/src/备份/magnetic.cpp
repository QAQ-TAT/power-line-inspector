...
void redUDP()
{
    ...
    while(1)
    {
        ...
        cout << "AutoForwardFlag=" << AutoForwardFlag << endl;
        ...
        else if(ret_magnetic == 0 )
        {
            ...
            if (flag_lowbattery == 0)//非低电量时 
            {
                if(cardid == 0x41e801A)
                {
                    ...
                    //cardid = 0;//一套流程以后 读卡数据要清零 不然出轨以后 会自己行动
                ...
                // esle if(strncmp(cardidstr,"4187e1a",7) == 0)
                // {
                //     cout << "这里是4187e1a+++++++++++++++++++++++++++++++++++++++++++++++++++++++" << endl;
                // }
...
/*****************************************************************************功能函数**********************************************/
    thread Magneticthread(redMagneticSerial);
    thread Rfidthread(redRfidSerial);
    thread USBCANthread(redUSBCanSerial);
    thread USBIICthread(redUSBIICSerial);
    thread UDPthread(redUDP);
    thread processDATAthread(processData_magnetic);
    ...
    while(n.ok())
    {
        ...
        //自动前进循迹
        if(AutoForwardFlag == 1) //if(AutoForwardFlag[1] == '1' /*&& AutoBackFlag[1] == '0'*/ )
        {
        ...
        //停止自动前进寻迹
        else if(AutoForwardFlag == 0 && AutoForwardStopFlag == 1 /*&& AutoBackFlag[1] == '0'*/)//else if(AutoForwardFlag[1] == '0' && AutoForwardStopFlag == 1 /*&& AutoBackFlag[1] == '0'*/)//UDP非主动发送 避免上位机重启以后标志位不匹配
        {
            ret_magnetic = 0;//停止自动寻迹后 磁力传感器接收的数据清零  ******这里是重点*****
            AutoForwardPolicy();
            cout << __LINE__ << endl;
            AutoForwardStopFlag = 0;
        }

        //自动后退寻迹
        else if(AutoBackFlag == 1 && AutoForwardFlag == 0)//else if(AutoBackFlag[1] == '1' && AutoForwardFlag[1] == '0')
        {
            ret_magnetic = strtol(hex2decbuf,&ptr,0);//开启自动循迹 //hex2decbuf 磁传感器发过来的值 字符串的123 转换成十进制123
            AutoBackPolicy();
        } 

        //停止自动后退寻迹
        else if(AutoForwardFlag == 0 && AutoBackStopFlag == 1 && AutoBackFlag == 0)//else if(AutoForwardFlag[1] == '0' && AutoBackStopFlag == 1 && AutoBackFlag[1] == '0')//UDP非主动发送 避免上位机重启以后标志位不匹配
        {
            ret_magnetic = 0;//停止自动寻迹后 磁力传感器接收的数据清零  ******这里是重点*****
            AutoBackPolicy();
            cout << __LINE__ << endl;
